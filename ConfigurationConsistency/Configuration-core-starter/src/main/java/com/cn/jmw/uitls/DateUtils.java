...
public class DateUtils {
    ...
    public static final Long DATE_PRESENT = -999999999999L; // 至今
...
//--格式化日期start-----------------------------------------
    ...
    public static Date parse(String text) {
        ...
        Date d = parseDateTime(text);
        if (d != null) {
            return d;
        } else {
            d = parseDateTimeMinute(text);
            if (d != null) {
                return d;
            } else {
                d = parseDate(text);
    ...
    public static Date parseDateTimeMinute(String text) {
    ...
    public static Date parseMonth(String text) {
        SimpleDateFormat format = new SimpleDateFormat(DEFAULT_MONTH_FORMAT);
        try {
            return format.parse(text);
        } catch (Exception e) {
            return null;
     ...
     * @return
     */
    public static long untilMinutes(Date endDate) {
        return date2LocalDateTime(endDate).until(LocalDateTime.now(), ChronoUnit.MINUTES);
    }
...
//----------//----------//----------//----------//----------//----------//----------//----------//----------//----------//----------
     ...
     * @return 结果
     */
    public static boolean between(Date from, Date to) {
        if (from == null) {
        ...
        return now.isAfter(date2LocalDateTime(from)) && now.isBefore(date2LocalDateTime(to));
    }
    /**
     * 判断当前时间是否在指定时间范围
     *
    ...
    public static Year year(Date date) {
    ...
    public final static boolean isEquals(Long l1, Long l2) {
        if (l1 == null || l2 == null) {
            return false;
        }
     ...
     * @return 结果
     */
    public static boolean before(Date to) {
        ...
        return now.isBefore(date2LocalDateTime(to));
    }
